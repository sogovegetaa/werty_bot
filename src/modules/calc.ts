...
export const calcModule = async (msg: Message): Promise<void> => {
        ...
        try {
            for (let i = 0; i < multiMatches.length; i++) {
                ...
                const innerNormalized = innerExpr
                    .replace(/,/g, ".")
                    .replace(/[–—−]/g, "-");
                let innerWithPercent = innerNormalized.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_mm, a: string, op: string, b: string) => `(${a}${op}(${a}*(${b}/100)))`);
                innerWithPercent = innerWithPercent.replace(/(\d+(?:\.\d+)?)%/g, (_mm, x: string) => `((${x})/100)`);
            ...
            let withPercentFinal = exprForCalc.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_m, a: string, op: string, b: string) => `(${a}${op}(${a}*(${b}/100)))`);
            withPercentFinal = withPercentFinal.replace(/(\d+(?:\.\d+)?)%/g, (_m, x: string) => `((${x})/100)`);
    ...
    let withPercent = normalized.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_m, a: string, op: string, b: string) => `(${a}${op}(${a}*(${b}/100)))`);
