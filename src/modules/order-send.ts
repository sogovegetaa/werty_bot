...
export const orderSendModule = async (msg: Message): Promise<void> => {
  ...
  try {
          ...
          if (convertedText) {
            ...
            if (numberMatch) {
              ...
              if (!order) {
                await bot.sendMessage(
                  chatId,
                  `❌ Заявка не найдена. Результат конвертации: ${convertedValueNum.toFixed(2)} ${quote}.`
                );
                return;
              }
              ...
              const remaining_usdt = Math.max(
                Number(order.usdt_amount || 0) - sent_usdt,
                0
              );
              ...
              await bot.sendMessage(
                chatId,
                `✅ Отправлено: <code>${convertedValueNum.toFixed(2)}</code> USDT (${conversionInfo})\n` +
    ...
    let messageText =
      `💰 Отправлено: <code>${amount.toLocaleString("ru-RU")}</code> USDT\n\n` +
      `Заявка: <b>${order.id}</b>\n` +
      `Всего по заявке: <code>${order.usdt_amount.toLocaleString(
        "ru-RU"
      )}</code> USDT\n` +
      `Уже отправлено: <code>${sent_usdt.toLocaleString(
        "ru-RU"
      )}</code> USDT\n` +
      `Осталось: <code>${remaining_usdt.toLocaleString(
        "ru-RU"
      )}</code> USDT\n\n`;
