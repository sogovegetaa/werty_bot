...
type Scope = "mine" | "all";
type OverrideUser = { telegramId: number; username?: string };

export const sendOrdersReportXls = async (
  msg: Message,
  scope: Scope,
  ...
  try {
    ...
    let query = supabase
      .from("order")
      .select(
        "id, rub_get, rub_give, rate, usdt_amount, sent_usdt, remaining_usdt, status, created_at, updated_at, user_id"
      )
      .eq("chat_id", chatId)
