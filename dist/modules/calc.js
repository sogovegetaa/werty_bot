...
export const calcModule = async (msg) => {
                ...
                if (convertedText) {
                    ...
                    if (numberMatch) {
                        ...
                        try {
                            ...
                            await bot.sendMessage(chatId, `<code>${amount} ${base} → ${quote} = ${formattedConverted}</code>\n` +
                                `<code>${formattedConverted} ${normalizedMathExpr.replace(/\s+/g, "")} = ${formattedResult}</code>`, { parse_mode: "HTML" });
                            return;
                        }
                        catch (err) {
                ...
                try {
                    ...
                    if (convertedText) {
                        ...
                        if (numberMatch) {
                            ...
                            await bot.sendMessage(chatId, `<code>${resultMessage}</code>`, { parse_mode: "HTML" });
                            return;
                        }
                    }
                    await bot.sendMessage(chatId, `❌ Не удалось получить данные с XE.com.`);
    ...
    const rawExpr = (mNamed ? mNamed[1] : mDirect[1]) || "";
        ...
        try {
            for (let i = 0; i < multiMatches.length; i++) {
                ...
                const innerNormalized = innerExpr
                    .replace(/,/g, ".")
                    .replace(/[–—−]/g, "-");
                let innerWithPercent = innerNormalized.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_mm, a, op, b) => `(${a}${op}(${a}*(${b}/100)))`);
                innerWithPercent = innerWithPercent.replace(/(\d+(?:\.\d+)?)%/g, (_mm, x) => `((${x})/100)`);
            ...
            let withPercentFinal = exprForCalc.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_m, a, op, b) => `(${a}${op}(${a}*(${b}/100)))`);
            withPercentFinal = withPercentFinal.replace(/(\d+(?:\.\d+)?)%/g, (_m, x) => `((${x})/100)`);
            ...
            await bot.sendMessage(chatId, `<code>${lines.join("\n")}</code>\n\n` +
                `<code>${displayExprFinal}</code> = <code>${formattedFinal}</code>`, { parse_mode: "HTML" });
            return;
        }
        catch (err) {
    ...
    let withPercent = normalized.replace(/(\d+(?:\.\d+)?)\s*([+\-])\s*(\d+(?:\.\d+)?)%/g, (_m, a, op, b) => `(${a}${op}(${a}*(${b}/100)))`);
    withPercent = withPercent.replace(/(\d+(?:\.\d+)?)%/g, (_m, x) => `((${x})/100)`);
    ...
    try {
        ...
        await bot.sendMessage(chatId, `<code>${displayExpr}</code> = <code>${formatted}</code>`, { parse_mode: "HTML" });
    }
    catch (err) {
