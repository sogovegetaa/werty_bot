...
export const createOrderModule = async (msg) => {
    ...
    try {
        ...
        if (!parts) {
            await bot.sendMessage(chatId, "Формат: /пр <code>{руб_получаем}</code> от <code>{руб_отдаем}</code>/<code>{курс}</code>", { parse_mode: "HTML" });
            return;
        }
        ...
        const messageText = `Заявка: <b><code>${order?.id}</code></b>\n` +
            `----\n` +
