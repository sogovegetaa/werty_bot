...
export const updateOrderModule = async (msg: Message): Promise<void> => {
  ...
  try {
    ...
    if (newFmt) {
      const rubGet = parseFloat(
        newFmt[2].replace(/\s+/g, "").replace(",", ".")
      );
      const rubGive = parseFloat(
        newFmt[3].replace(/\s+/g, "").replace(",", ".")
      );
      ...
      if ([rubGet, rubGive, rate].some((n) => Number.isNaN(n) || n <= 0)) {
        await bot.sendMessage(
          chatId,
          "❌ Проверьте числа: /изм <id> <руб_получаем> от <руб_отдаем>/<курс>",
          { parse_mode: "HTML" }
        );
        return;
      }
    ...
    if (newOrder.chat_id && newOrder.message_id) {
      const messageText =
        `Заявка: <b><code>${newOrder.id}</code></b>\n` +
        `----\n` +
        `Получаем: <code>${newOrder.rub_get.toLocaleString(
          "ru-RU"
        )}</code> руб\n` +
        `Курс: <code>${newOrder.rate}</code>\n` +
        `Отдаем: <code>${newOrder.usdt_amount.toLocaleString(
          "ru-RU"
        )}</code> юсдт\n` +
